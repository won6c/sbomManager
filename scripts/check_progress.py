...
def get_progress_files(root_dir: str):
    """Find all tasks/progress.json files in the project."""
    return Path(root_dir).rglob("tasks/progress.json")
...
def format_progress(name: str, data: Dict[str, Any]) -> str:
    ...
    # Assuming we might add a 'todo' list to tasks/progress.json in the future
    todo = data.get("todo", [])
...
def main():
    ...
    root_progress_file = Path(root_path) / "tasks/progress.json"
    ...
    for log_file in Path(root_path).rglob("tasks/done/SESSION_LOG.md"):
